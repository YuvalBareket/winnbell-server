...
export const grantPendingReferralBonus = async (
  ...
  // Welcome-bonus attribution + reward state live in user_acquisition (1:1 with the user). Only a
  // person referral grants a welcome entry: referred_by_user_id set (referral_rewarded_at guards it,
  // so it's one-per-user). Location flyer/QR signups do NOT get a welcome entry - they earn entries
  // by submitting a receipt like any organic visitor.
  ...
  // entries or cap. entry_source='referral' marks it a welcome bonus, distinct from a self-claimed
  // AMOE 'free' entry.

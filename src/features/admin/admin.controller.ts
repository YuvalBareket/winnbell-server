...
import { getRulesPdfForAdmin } from '../../shared/legal/officialRulesSnapshot.js';

// Admin download of a draw's Official Rules PDF. Closed draws serve the ARCHIVED
// close-time PDF byte-exact (the legal record; regenerating could differ if the
// template or region settings changed since) - backfilled if the archive is missing.
// Open/Upcoming draws generate fresh from the live rules.
export const downloadDrawRulesPdf = async (req: Request, res: Response) => {
  ...
  try {
    const { pdf, filename, source } = await getRulesPdfForAdmin(drawId);
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', `attachment; filename="${filename}"`);
    res.setHeader('X-Rules-Source', source);
    res.send(pdf);
  } catch (error) {
    ...
    console.error('Rules PDF retrieval error:', error instanceof Error ? error.message : error);
    res.status(500).json({ message: 'Failed to retrieve rules PDF' });
  }
};

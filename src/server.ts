...
const startServer = async () => {
  try {
    ...
    // Heal subscription rows whose Stripe webhooks were lost (server down past the retry
    // window): every 6 hours AND once shortly after boot - a restart is exactly when
    // deliveries were being missed, so heal immediately instead of waiting 6 hours.
    setTimeout(() => {
      reconcileSubscriptionsWithStripe().catch(err => console.error('[Stripe reconcile] boot run failed:', err));
    }, 60 * 1000);
    setInterval(() => {
      reconcileSubscriptionsWithStripe().catch(err => console.error('[Stripe reconcile] periodic run failed:', err));
    }, 6 * 60 * 60 * 1000);
